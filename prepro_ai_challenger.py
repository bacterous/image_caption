...
import jieba   #jieba to cut the word

def prepro_captions(imgs):
    # preprocess all the captions
    print 'example processed tokens:' ### fuck note wrong code from Github
    #print len(imgs),type(imgs)
    #print imgs
    #print imgs.keys()
    #print type(imgs[0]),type(imgs[1])
...
def build_vocab(imgs, params):
    ...
    for img in imgs:
        ...
        for sent in img['sentences']:
            txt = sent['tokens']
            caption = [w if counts.get(w,0) > count_thr else 'UNK' for w in txt]  # keep the common tokens for each sentence 4
            img['final_captions'].append(caption)


    #print (imgs[2]["processed_tokens"])
    #print (imgs[2]['final_captions'])
    #print (imgs[200]['final_captions'])
    #exit()
...
def assign_splits(imgs, params):
  ...
  for i,img in enumerate(imgs):
      if 'val' in img['filepath']: #fuck code
        img['split'] = 'val'
      else:
        img['split'] = 'train'
    ...
    for i, img in enumerate(imgs):
        ...
        for j, s in enumerate(img['final_captions']):
            ...
            for k,w in enumerate(s):
                if k < max_length:
                    Li[j,k] = wtoi[w]  #j-th sentence ,k-th token -> store the token's index
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/joey/data/coco_ai_challenger.json',
                        help='input json file to process into hdf5')
    parser.add_argument('--num_val', default=30000, type=int,
                        help='number of images to assign to validation data (for CV etc)')
    parser.add_argument('--output_json', default='/home/joey/data/coco_ai_challenger_talk.json', help='output json file')
    parser.add_argument('--output_h5', default='/home/joey/data/coco_ai_challenger_talk_v2', help='output h5 file')

    # options
    parser.add_argument('--max_length', default=32, type=int,
                        help='max length of a caption, in number of words. captions longer than this get clipped.')
    parser.add_argument('--images_root', default='/home/joey/data',
